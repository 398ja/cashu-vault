...
The application can be started directly on your machine once it has been built. Execute the jar produced by the build or run it from source:
...
Datasource settings can be customised with the environment variables defined in `docker-compose.yml`.
...
`VaultClient` reads the base URL of the service from the `VAULT_BASE_URL` environment variable or the `vault.base.url` system property. If neither is set, it falls back to `http://localhost:${cashu_vault_port}` with a default port of `3333`.
The provided Docker image defines `cashu_vault_port` so the service defaults to `http://localhost:3333` when started with Docker Compose.
